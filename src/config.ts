...
export function mergeConfig(
  ...
  return {
    ...
    maxRoundsPerRun: override.maxRoundsPerRun ?? base.maxRoundsPerRun ?? 20,
    ...
    decisionNudgeMinutes: override.decisionNudgeMinutes ?? base.decisionNudgeMinutes ?? 5,
...
export function validateConfig(value: unknown): string | undefined {
  ...
  const ranges: Record<string, [number, number]> = {
    ...
    decisionNudgeMinutes: [0, 240],
    maxLogBytesPerRun: [0, 1_000_000_000],
    providerQuotaWaitMinutes: [0, 1_440],
...
export function describeConfig(config: MaestroConfig): string {
  const lines = [
    ...
    `maxRoundsPerRun: ${config.maxRoundsPerRun ?? 20}`,
