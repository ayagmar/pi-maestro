...
/**
 * Follow-up for an attempt cut off by the cost cap and continued in the same
 * session after the operator raised it. Nothing was judged wrong; the model
 * only needs to know its edits survived and that it should finish, not restart.
 */
export function buildCostCapResumePrompt(task: Task, spent: number): string {
  const criteria = task.successCriteria?.length
    ? `\n\n## Success criteria (unchanged)\n${task.successCriteria.map((criterion, index) => `${index + 1}. ${criterion}`).join("\n")}`
    : "";
  return joinBudgetedSections([
    `Your previous attempt on Task ${task.id} (${task.title}) was stopped by the per-attempt cost cap after $${spent.toFixed(2)}, not because anything was wrong. The cap has been raised and this is a continuation of the same attempt: every edit you made is still in the working tree (checkpointed on this branch). Continue from where you stopped rather than starting over or re-reading what you already know.${criteria}`,
    [
      "## Stop rule",
      "Finish the remaining work, run the most relevant verification, and check every acceptance criterion. Be economical: this continuation has its own cost cap.",
      "",
      "## Report",
      "End your final message with a `## Report` section covering the whole task: what was done across both parts of the attempt, files changed, how it was verified, open questions or risks.",
    ].join("\n"),
  ]);
}
...
export function buildSupervisorBriefing(
  ...
  return [
    ...
    [
      ...
      "",
      "## Workflow",
      "1. Call `maestro_drive` action=start once. It runs the whole mechanical run/review/retry loop, in parallel where dependencies allow, and returns only when the board is settled or a decision needs your judgment.",
      "2. Act on the text that call returns. Never poll a running drive with `sleep`, bash waits, or repeated `inspect` calls: each one costs a turn, re-sends the conversation, and evicts the prompt cache while changing nothing.",
      ...
      "",
      "## Decision rules",
      "- `maestro_drive` action=start already reports the settled outcome; only use inspect when you did not start the drive yourself or the user asks for current state. Never read `.pi/maestro` internals, agent session/log files, or write ad-hoc inspection scripts: those transcripts are megabytes of replayed tool output and reading one destroys your context and prompt cache. Recorded sessions are for the human dashboard, not for you.",
...
export function buildOrchestratorBriefing(
  ...
  return [
    ...
    [
      ...
      `3. \`maestro_drive\` action=start: runs the whole mechanical run/review/retry loop, in parallel where dependencies allow, and returns only when the board is settled or a decision needs your judgment.${planGate ? " When planGate is enabled, it refuses with plan_gate until the user approves via /maestro plan; announce the pending plan and wait instead of retrying." : ""}`,
      "4. Act on the text that call returns. Never poll a running drive with `sleep`, bash waits, or repeated `inspect` calls: each one costs a turn, re-sends the conversation, and evicts the prompt cache while changing nothing.",
      ...
      "- Executors run in isolated checkouts built from committed content. If a brief points at a file, make sure it is committed; uncommitted files do not exist for the executor.",
      "- At a provider block: switch to a configured fallback then `/maestro resume`, or ask the user if it is a quota/cost decision. Never blindly retry the same blocked provider.",
      ...
      "- `maestro_update` rejects contract edits to an in-flight attempt (running or ready_for_review) unless `invalidateInFlight: true` explicitly accepts the recorded cost and discarded work; prefer letting it settle.",
      "- At an attempt cap, create one narrowly scoped successor with `maestro_plan` and set `supersedesTaskId` to the capped task. Maestro atomically keeps the predecessor visible as cancelled and rewires every downstream dependency. Then call `maestro_drive` action=start for the successor and rewired dependents. Never perform cancellation and rewiring as separate calls.",
      "- Ask the user before expanding scope beyond the stated goal or making a cost tradeoff.",
      "- After changing a brief/tier, `/maestro resume` to continue the drive.",
