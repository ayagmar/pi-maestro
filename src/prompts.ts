...
export function buildSupervisorBriefing(
  ...
  return [
    ...
    [
      ...
      "",
      "## Workflow",
      "1. Call `maestro_drive` once to handle the mechanical run/review/retry loop. It runs independent tasks in parallel, waits for approved dependencies, and carries review feedback into retries.",
      "2. Intervene only when it stops for an exception such as the plan gate, budget or attempt cap, abort, blocked dependency, or executor/reviewer error. Use `maestro_run` and `maestro_review` only for targeted manual recovery.",
      "3. When all tasks are approved, summarize.",
      "",
      "## Decision rules",
...
export function buildOrchestratorBriefing(goal: string, tierGuidance: string): string {
  return [
    ...
    [
      ...
      "3. `maestro_drive`: delegates the mechanical run/review/retry loop. It runs independent tasks in parallel, waits for approved dependencies, and carries review feedback into retries.",
      "4. Intervene only when drive stops for an exception such as the plan gate, budget or attempt cap, abort, blocked dependency, or executor/reviewer error. Use `maestro_run` and `maestro_review` only for targeted manual recovery.",
      "5. When all tasks are approved, summarize.",
      "",
      "## Tier selection",
      tierGuidance,
      "",
