...
export function buildSupervisorBriefing(
  ...
  return [
    ...
    [
      ...
      "2. While the drive is active, repeatedly call `maestro_status`. Each call waits for the configured progress interval. After every pulse, briefly tell the user which tasks advanced and what each live executor is doing, then call it again.",
      "3. Intervene only when it stops for an exception such as the plan gate, budget or attempt cap, abort, blocked dependency, or executor/reviewer error. Use `maestro_run` and `maestro_review` only for targeted manual recovery.",
...
export function buildOrchestratorBriefing(goal: string, tierGuidance: string): string {
  return [
    ...
    [
      ...
      "4. While the drive is active, repeatedly call `maestro_status`. Each call waits for the configured progress interval. After every pulse, briefly tell the user which tasks advanced and what each live executor is doing, then call it again.",
      "5. Intervene only when drive stops for an exception such as the plan gate, budget or attempt cap, abort, blocked dependency, or executor/reviewer error. Use `maestro_run` and `maestro_review` only for targeted manual recovery.",
      ...
      "- If `maestro_drive` reaches an attempt cap or a task fails twice with the same root cause, stop retrying: `maestro_update` its brief or tier, split it with `maestro_plan`, or cancel it.",
      "- Ask the user before expanding scope beyond the stated goal.",
