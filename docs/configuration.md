...
| `reviewCheapModel` | (none) | Model pattern for the first reviewer launch on each attempt. Unset keeps every reviewer on the `review` tier. The review tier takes over when the cheap first pass is doubtful (a rejection, no `VERDICT` line, or malformed criterion evidence), the change touches a money or migration/schema path, a refuter stage runs, or the trigger says so. |
