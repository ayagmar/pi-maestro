...
| `provider_blocked` / `provider_failure` | Transient network drops retry twice on their own (15 s, 60 s); an exhausted quota or usage window is probed with 2/4/8/15-minute backoff for `providerQuotaWaitMinutes` before the drive stops. When it does stop, fix credentials or configure a fallback, then `/maestro resume` from any session — this stop is mechanical and carries no owner guard. An attempt the provider cut off mid-work resumes its own session and checkout. | Failed launches, usage, provider reason, interrupted session and checkpoint branch. |
